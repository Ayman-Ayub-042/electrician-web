...
export default function ResidentialTbody(props) {
  ...
  const updatesubcat = async (uid) => {
    ...
    setIsLoading(true);
    const updatedocument = doc(db, `residential_code/${props.locc}/codes`,uid);
  ...
  const deletesubcat = async (uid) => {
    setIsLoading(true);
    const deletedocument = doc(db, `residential_code/${props.locc}/codes`,uid);
  ...
  return (
    <>
      <tr  style={{
        ...
        {isLoading ? <LoadingSpinner /> : ResidentialTbody}
      </tr>

      <Modal
        title="Edit Residential code text"
