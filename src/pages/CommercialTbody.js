...
export default function CommercialTbody(props) {
  const [isLoading, setIsLoading] = useState(false);
    const [showupdatesubCatModal, setShowupdatesubCatModal] = useState(false);
    const [textupdate, settextupdate] = useState(props.text);
    const [dateupdate, setdateupdate] = useState(props.date);
  ...
  const updatesubcat = async (uid) => {
    ...
    setIsLoading(true);
    const updatedocument = doc(db, `commercial_code/${props.locc}/codes`,uid);
  ...
  const deletesubcat = async (uid) => {
    setIsLoading(true);
    const deletedocument = doc(db, `commercial_code/${props.locc}/codes`,uid);
  ...
  return (
    <>
      <tr  style={{
        ...
        {isLoading ? <LoadingSpinner /> : CommercialTbody}
      </tr>

      <Modal
        title="Edit Commercial code text"
       ...
       <button
        ...
        <div className="input-container">
          ...
          <textarea
                      ...
                      name="subcategory"
                      id="subcategory"
                      class="bg-gray-50 border  border-gray-300 mt-3 text-gray-900 h-20 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"
