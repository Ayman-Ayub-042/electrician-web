...
import CommercialTbody from "./CommercialTbody";
...
const Commercial = () => {
    ...
    const getCommercialcode = async (loc) => {
      const addCommercial1 = collection(db, loc.length === 0 ? `commercial_code/grounding/codes` : `commercial_code/${loc}/codes`);
      console.log(addCommercial1.path);
      const getData = await getDocs(addCommercial1);
      // console.log(TableHeader)
     
      setCommercialData(getData.docs.map((doc) => ({ ...doc.data(), id: doc.id })));
    };
  
  
    const AddCommercial = async (props) => {
        const data = new Date();
        ...
        setIsLoading(true);
      const addCommercial2 = collection(db, addlocation.length === 0 ? `commercial_code/grounding/codes` : `commercial_code/${addlocation}/codes`);
      console.log(addCommercial2.path);
      const addData = await addDoc(addCommercial2, {
    ...
    useEffect(() => {
      var loc=localStorage.getItem('comlocation');
      if (loc){
        setLocation(loc)
      ...
      if (loc){
        getCommercialcode(loc)
        
      }
      else{
        getCommercialcode("grounding")
    ...
    return (
      <div className="" >
                ...
                <AiFillCaretDown className="icon small" /> */}
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      localStorage.setItem('comlocation',e.target.value);
                      getCommercialcode(e.target.value.toString())
        ...
        >
          ...
          {isLoading ? <LoadingSpinner /> : Commercial}
        </Modal>
      </div>
    );
  };
